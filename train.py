...
def sequence_loss(flow_preds, flow_gt, valid, gamma=0.8, max_flow=MAX_FLOW):
    ...
    for i in range(n_predictions):
        ...
        ref, sparse_flow, confidence = flow_preds[1][i]
        n = ref.shape[1]
        ...
        sparse_i_loss = (sparse_flow * scale - sparse_gt).abs() * confidence
    ...
    def write_images(self, image1, image2, targets, preds):
        ...
        for n_i in range(len(targets)):
            ...
            for p_i in range(len(preds[0])):
                ...
                confidence = np.suqeeze(confidence.detach().cpu().numpy()[n_i])
                ref_img = cv2.cvtColor(np.array(this_image1, dtype=np.uint8), cv2.COLOR_RGB2BGR)
                for k_i in range(len(coords)):
                    coord = coords[k_i]
                    ref_img = cv2.circle(ref_img, coord, 10, (np.round(255 * confidence[k_i]), 0, 0), 10)
