...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=128, nhead=8,
                 num_encoder_layers=6, num_decoder_layers=6, dim_feedforward=128 * 4, dropout=0.1,
        ...
        self.prop_decoder = DeformableTransformerDecoder(decoder_layer, 1, return_intermediate_dec)
    ...
    def forward(self, srcs_01, srcs_02, pos_embeds):
        ...
        prop_pos = torch.cat((lvl_pos_embed_flatten, prop_tgt_N_query_pos.repeat(bs, 1, 1)), dim=1)
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, query_pos, reference_points,
        ...
        tgt2, scores = self.cross_attn(self.with_pos_embed(tgt, query_pos), reference_points,
                                       self.with_pos_embed(src, src_pos), src_spatial_shapes, level_start_index)
        # tgt2, scores = self.cross_attn(self.with_pos_embed(tgt, query_pos), reference_points,
